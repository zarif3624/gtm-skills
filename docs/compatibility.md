...
| --- | --- | --- | --- |
| `skills` CLI against this repository | `npx -y skills add . --list` discovered all 24 skills locally | Not applicable | Verified locally on 2026-08-08 |
...
| OpenAI Codex clean-context subagent | Skills loaded from their repository paths; the latest blind metadata-only run selected the exact intended minimal set for 27/27 routing requests with zero excluded neighbors | Seven latest recorded behavioral scenarios pass, including pipeline→forecast, sparse forecast, source-injection, coaching, customer outcomes, business-case, and sales-process risks; exact model identifier was unavailable | Limited positive evidence for the recorded lineage and commits; see [behavior history](../evals/results/) and [routing evidence](../evals/routing/results/) |

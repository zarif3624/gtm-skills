...
| --- | --- | --- | --- |
| `skills` CLI against this repository | `npx -y skills add . --list` discovered all 25 skills; isolated `--copy` installs of `research-account` and resource-bearing `forecast-sales` preserved every package file byte for byte | Not applicable | Verified locally on 2026-08-08; installation from a published tag remains a release check |
